...
if [ -f "814712_MTL_TuneD_Static_Profile_RN_Rev1p2.zip" ]; then
    unzip 814712_MTL_TuneD_Static_Profile_RN_Rev1p2.zip
else
    echo "File '814712_MTL_TuneD_Static_Profile_RN_Rev1p2.zip' not found. Exiting."
    exit 1
fi

if [ -f "pkg.OPT.EPPprofile-1.05.240206.1-x86_64.zip" ]; then
    unzip pkg.OPT.EPPprofile-1.05.240206.1-x86_64.zip
else
    echo "File 'pkg.OPT.EPPprofile-1.05.240206.1-x86_64.zip' not found. Exiting."
    exit 1
fi
...
curl -o deploy.sh https://raw.githubusercontent.com/ctsdownloads/testing/main/deploy.sh
if [ $? -ne 0 ]; then
    echo "Failed to download deploy.sh. Exiting."
    exit 1
fi
...
sudo bash deploy.sh
if [ $? -ne 0 ]; then
    echo "Failed to run deploy.sh. Exiting."
    exit 1
fi
...
sudo dnf install -y python3-gobject python3-cairo gobject-introspection gtk3 libappindicator-gtk3
if [ $? -ne 0 ]; then
